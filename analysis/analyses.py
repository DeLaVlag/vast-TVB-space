# https://github.com/davidaquilue/TVBAdEx_ParSweep/blob/main/JUSUFlike/Project/Codes/HPC_sim.py
...
def varFCD(FR, end_time, window=1000, overlap=100, k=2, do_plot=False):
    ...
    for i, t_i in enumerate(FC_time_serie):
        for j, t_j in enumerate(FC_time_serie):
            FCD[i, j] = np.corrcoef(t_i.ravel(), t_j.ravel())[0, 1]
    if do_plot:
        im = plt.imshow(FCD)
...
def frequency_analysis(recorded_signal, fs, frequency_resolution=1.0):
    """ Returns frequency at which PSD peaks and the amplitude of the peak. Makes use of scipy.signal.find_peaks.
    ...
    frequency_resolution: output resolution

    recorded_signal: array_like
        time series
    ...
    """
    # welch_model = welch_psd(recorded_signal, frequency_resolution=frequency_resolution, fs=fs, axis=0)
